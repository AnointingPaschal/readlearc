...
import {
  ...
  PenLine, FileText, FlaskConical, Hash,
} from "lucide-react";
...
export default function WritePage() {
  ...
  const TOOLBAR = [
    ...
    { icon:Heading2,   title:"Heading", action:()=>insertText("\n## ","","Heading")          },
  ...
  if (!isConnected) return (
    <div style={{ minHeight:"100vh", background:"var(--bg)" }}>
      <SetupBanner/><Navbar/>
      <ConnectGate title="Connect to write" body="Connect your wallet to publish articles on-chain to Arc blockchain." icon={PenLine}/>
    </div>
  );
      ...
      <div style={{ maxWidth:1160, margin:"0 auto", padding:"calc(var(--header-h) + 20px) 14px 60px" }}>
          ...
          <div style={{ display:"flex", gap:8, alignItems:"center" }}>
            ...
            <button onClick={publish}
              ...
              {publishing
                ...
                : <><Send size={13}/>Publish to Arc</>}
            </button>
          </div>
        </div>

        {error && (
          <div style={{ marginBottom:14, padding:"12px 16px", background:"rgba(220,38,38,.06)", border:"1px solid rgba(220,38,38,.18)", borderRadius:"var(--r-md)", display:"flex", gap:9, alignItems:"flex-start" }}>
            <AlertCircle size={14} style={{ color:"#dc2626", flexShrink:0, marginTop:1 }}/>
            <span style={{ fontSize:13, color:"#dc2626", lineHeight:1.6 }}>{error}</span>
          </div>
        )}
        ...
        {published ? (
          <div className="card" style={{ padding:"clamp(32px,6vw,64px) clamp(20px,4vw,32px)", textAlign:"center", maxWidth:520, margin:"0 auto" }}>
            ...
            <h2 style={{ fontFamily:"Outfit,sans-serif", fontSize:"clamp(22px,5vw,32px)", fontWeight:900, color:"var(--text)", marginBottom:10, letterSpacing:"-0.02em" }}>
              {isResearch ? "Research paper" : "Article"} published!
            </h2>
            <p style={{ color:"var(--text-3)", fontSize:14, marginBottom:6 }}>Live on Arc blockchain · Article #{articleId}</p>
            {txHash && (
              <a href={`${EXPLORER_URL}/tx/${txHash}`} target="_blank" rel="noopener noreferrer"
                style={{ fontSize:11, color:"var(--brand)", fontFamily:"JetBrains Mono,monospace", textDecoration:"none", display:"inline-flex", alignItems:"center", gap:3, marginBottom:24 }}>
            ...
            <div style={{ display:"flex", gap:10, justifyContent:"center", flexWrap:"wrap", marginTop:16 }}>
              ...
              <button onClick={() => { setPublished(false); setTitle(""); setBlurb(""); setBody(""); setCategory(""); setTxHash(""); setArticleId(""); setIsResearch(false); }} className="btn btn-ghost">Write Another</button>
            </div>
          </div>
        ) : (
          /* ── Editor layout ── */
          <div style={{ display:"grid", gridTemplateColumns:"1fr 260px", gap:16, alignItems:"start" }} id="write-grid">
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
              ...
              <div className="card" style={{ padding:"20px 22px", overflow:"hidden" }}>
                <input
                  type="text"
                  placeholder={isResearch ? "Research paper title…" : "Article title…"}
                  value={title}
              ...
              <div className="card" style={{ padding:"16px 20px" }}>
                <label style={{ fontSize:10, fontWeight:700, color:"var(--text-4)", textTransform:"uppercase", letterSpacing:".07em", display:"block", marginBottom:7, fontFamily:"Outfit,sans-serif" }}>
                  Preview blurb <span style={{ color:"var(--text-4)", fontWeight:400, fontSize:9 }}>(shown before payment gate)</span>
                </label>
                <textarea
                  placeholder="A teaser that convinces readers to unlock the full article…"
                  value={blurb}
              ...
              {!preview ? (
                <div className="card" style={{ overflow:"hidden", padding:0 }}>
                  ...
                  <textarea
                    ...
                    placeholder={isResearch
                      ...
                      : "Write your full article here. Markdown is supported: **bold**, _italic_, ## heading, > quote, - list…\n\nThe first 50% will be shown as a free preview to readers."}
                    value={body}
                    ...
                    style={{
                      ...
                      padding:"20px 22px", resize:"vertical", minHeight:400,
                    }}
                  />
                </div>
              ) : (
                ...
                <div className="card" style={{ padding:"clamp(20px,4vw,36px)" }}>
                  ...
                  <h1 style={{ fontFamily:"Outfit,sans-serif", fontSize:"clamp(22px,5vw,36px)", fontWeight:900, color:"var(--text)", marginBottom:16, lineHeight:1.15, letterSpacing:"-0.03em" }}>
                    {title || "Your title…"}
                  </h1>
                  <p style={{ fontSize:16, color:"var(--text-2)", lineHeight:1.72, marginBottom:20, borderLeft:"3px solid var(--brand)", paddingLeft:16, fontStyle:"italic" }}>
                    {blurb || "Your blurb…"}
                  </p>
                  <hr className="divider" style={{ marginBottom:20 }}/>
                  <div className="article-body"
                    dangerouslySetInnerHTML={{ __html: renderPreview(body) || "<p style='color:var(--text-4);font-style:italic'>Start writing to see preview…</p>" }}
                  />
                </div>
              )}
            </div>
            ...
            <div id="write-sidebar" style={{ display:"flex", flexDirection:"column", gap:12, position:"sticky", top:"calc(var(--header-h) + 12px)" }}>
              ...
              <div className="card" style={{ padding:"16px" }}>
                ...
                <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:7 }}>
                  {[
                    { id:false, icon:FileText,    label:"Article",  sub:"Blog post" },
                    { id:true,  icon:FlaskConical,label:"Research", sub:"Paper"     },
                  ].map(t => (
              ...
              <div className="card" style={{ padding:"16px" }}>
                <div style={{ display:"flex", alignItems:"center", gap:6, marginBottom:12 }}>
                  ...
                  <h3 style={{ fontSize:12, fontWeight:700, color:"var(--text-2)", fontFamily:"Outfit,sans-serif" }}>Price</h3>
                </div>
                <div style={{ display:"flex", alignItems:"baseline", gap:4, background:"var(--bg-alt)", border:"1.5px solid var(--border)", borderRadius:"var(--r-md)", padding:"8px 12px", marginBottom:10 }}>
                  ...
                  <input
                    type="number" min={0.001} max={1} step={0.001} value={price}
                ...
                />
                <div style={{ display:"flex", justifyContent:"space-between", fontSize:10, color:"var(--text-4)", marginBottom:10 }}>
                ...
                <div style={{ display:"flex", flexWrap:"wrap", gap:5 }}>
                  {CATS.map(c => (
                    <button key={c} onClick={() => setCategory(c)}
                      style={{ padding:"5px 10px", borderRadius:"var(--r-f)", fontSize:11, fontWeight:600, cursor:"pointer", transition:"all .13s",
                        border:`1.5px solid ${category===c?"var(--brand)":"var(--border)"}`,
                        background:category===c?"var(--brand-muted)":"transparent",
                        color:category===c?"var(--brand)":"var(--text-3)" }}>
                      {c}
                    </button>
                  ))}
                </div>
              ...
              <div className="card" style={{ padding:"16px" }}>
                <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:10 }}>
                  ...
                  <span style={{ fontSize:11, fontWeight:700, color:allDone?"var(--accent)":"var(--text-4)" }}>
                  ...
                  {checks.map(item => (
                    <div key={item.label} style={{ display:"flex", alignItems:"center", gap:8 }}>
                      <div style={{ width:16, height:16, borderRadius:"50%", flexShrink:0, display:"flex", alignItems:"center", justifyContent:"center",
                        border:`1.5px solid ${item.ok?"var(--accent)":"var(--border-2)"}`,
                        background:item.ok?"var(--accent-muted)":"transparent" }}>
