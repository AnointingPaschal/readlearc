...
import { ArrowLeft, BookOpen, DollarSign, Users, Clock, CheckCircle2, ExternalLink, Globe, PenLine, RefreshCw } from "lucide-react";
import { motion } from "framer-motion";
import Navbar from "../../../components/ui/Navbar";
import SetupBanner from "../../../components/ui/SetupBanner";
import { useWallet } from "../../../lib/wallet";
import { fetchArticlesByAuthor, fetchWriterStats, EXPLORER_URL, type Article } from "../../../lib/chain";
...
function loadProfile(addr: string) { try { return JSON.parse(localStorage.getItem(`rl-profile-${addr.toLowerCase()}`)||"{}"); } catch { return {}; } }

export default function ProfilePage() {
  const { handle }         = useParams<{ handle: string }>();
  const profileAddr        = handle;
  const { address, isConnected, provider } = useWallet();
  const isOwn              = isConnected && address.toLowerCase() === profileAddr?.toLowerCase();
  ...
  async function load() {
    ...
    const stats = await fetchWriterStats(profileAddr, provider||undefined);
      ...
      <div style={{ maxWidth:780, margin:"0 auto", padding:"76px 16px 60px" }}>
        ...
        <motion.div initial={{ opacity:0,y:14 }} animate={{ opacity:1,y:0 }} className="card" style={{ padding:"clamp(18px,4vw,28px)", marginBottom:16 }}>
          <div style={{ display:"flex", flexWrap:"wrap", gap:18, alignItems:"flex-start" }}>
            <div style={{ width:68,height:68,borderRadius:18,background:grad,flexShrink:0,boxShadow:"var(--shadow)" }}/>
            <div style={{ flex:1, minWidth:200 }}>
              ...
              <div style={{ display:"flex", gap:12, flexWrap:"wrap" }}>
                {profile.website && <a href={profile.website} target="_blank" rel="noopener noreferrer" style={{ display:"flex", alignItems:"center", gap:4, fontSize:12, color:"var(--brand)", textDecoration:"none", fontWeight:600 }}><Globe size={11}/>Website</a>}
                <a href={`${EXPLORER_URL}/address/${profileAddr}`} target="_blank" rel="noopener noreferrer" style={{ display:"flex", alignItems:"center", gap:4, fontSize:12, color:"var(--text-4)", textDecoration:"none" }}><ExternalLink size={10}/>On-chain</a>
              </div>
            </div>
            <div style={{ display:"flex", gap:16, flexWrap:"wrap", alignItems:"flex-start" }}>
              {loading ? [1,2,3].map(i=><div key={i} className="skeleton" style={{ width:60,height:40,borderRadius:8 }}/>) :
                [
          ...
          {isOwn && (
            <div style={{ marginTop:16, paddingTop:14, borderTop:"1px solid var(--border)", display:"flex", gap:8, flexWrap:"wrap" }}>
              ...
              <Link href="/write"   className="btn btn-primary btn-sm" style={{ fontWeight:700 }}><PenLine size={12}/>New Article</Link>
            </div>
          )}
        </motion.div>
        ...
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:12 }}>
          <h2 style={{ fontFamily:"Outfit,sans-serif", fontSize:16, fontWeight:700, color:"var(--text)" }}>Published Articles</h2>
          <button onClick={load} disabled={refreshing} style={{ width:30,height:30,borderRadius:"50%",border:"1.5px solid var(--border)",background:"var(--bg-alt)",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",color:"var(--text-3)" }}>
                  ...
                  <div className="card card-hover" style={{ padding:"16px 18px", display:"flex", alignItems:"flex-start", gap:12 }}>
                    <div style={{ flex:1, minWidth:0 }}>
                      ...
                      <div style={{ display:"flex", gap:10, fontSize:11, color:"var(--text-4)" }}>
                        ...
                        <span style={{ display:"flex", alignItems:"center", gap:2 }}><Users size={10}/>{a.reads} reads</span>
