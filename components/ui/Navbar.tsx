...
import { Zap, Sun, Moon, Menu, X, ChevronDown, LogOut, Copy, Check, Wallet, BookOpen, PenLine, LayoutDashboard, User, Lock } from "lucide-react";
...
export default function Navbar() {
  ...
  const { isAuth, short, address, balance, requireAuth, lock, disconnect } = useAuth();
  ...
  return (
    <>
      <nav style={{ position:"fixed", top:0, left:0, right:0, zIndex:50, height:"var(--header-h)", background:"var(--nav-bg)", backdropFilter:"blur(20px)", borderBottom:"1px solid var(--border)" }}>
        <div className="container" style={{ height:"100%", display:"flex", alignItems:"center", gap:10 }}>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:8 }}>
            ...
            {!isAuth ? (
              <button onClick={requireAuth} className="btn btn-primary" style={{ height:38, padding:"0 16px", gap:6, fontWeight:700 }}>
                <Wallet size={13}/>Sign In
              </button>
            ) : (
              <div ref={dropRef} style={{ position:"relative" }}>
                <button onClick={() => setDrop(v => !v)} style={{ display:"flex", alignItems:"center", gap:8, padding:"5px 10px 5px 5px", background:"var(--bg-alt)", border:"1.5px solid var(--border)", borderRadius:"var(--r-f)", cursor:"pointer" }}>
                ...
                {drop && (
                  <div style={{ position:"absolute", top:"calc(100% + 8px)", right:0, width:230, background:"var(--bg-card)", border:"1.5px solid var(--border)", borderRadius:"var(--r-lg)", boxShadow:"var(--shadow-lg)", overflow:"hidden", zIndex:100 }}>
                    {/* Header */}
                    <div style={{ padding:"12px 14px", background:"var(--bg-alt)", borderBottom:"1px solid var(--border)" }}>
                      <div style={{ fontSize:10, fontWeight:700, color:"var(--text-4)", textTransform:"uppercase", letterSpacing:".07em", marginBottom:3 }}>Platform Wallet</div>
                    ...
                    {[
                      { href:"/wallet-app",        icon:Wallet,         label:"My Wallet & Send"   },
                    ...
                    <button onClick={copy} style={{ width:"100%", display:"flex", alignItems:"center", gap:9, padding:"10px 14px", fontSize:13, color:"var(--text-3)", background:"none", border:"none", borderBottom:"1px solid var(--border)", cursor:"pointer", fontFamily:"inherit" }}
                      onMouseEnter={e => (e.currentTarget as any).style.background = "var(--bg-alt)"}
                      onMouseLeave={e => (e.currentTarget as any).style.background = ""}>
                    ...
                    <button onClick={lock} style={{ width:"100%", display:"flex", alignItems:"center", gap:9, padding:"10px 14px", fontSize:13, color:"#d97706", background:"none", border:"none", borderBottom:"1px solid var(--border)", cursor:"pointer", fontFamily:"inherit" }}
                      onMouseEnter={e => (e.currentTarget as any).style.background = "rgba(217,119,6,.05)"}
                      onMouseLeave={e => (e.currentTarget as any).style.background = ""}>
                      <Lock size={13}/>Lock Wallet
                    </button>

                    <button onClick={() => { disconnect(); setDrop(false); }} style={{ width:"100%", display:"flex", alignItems:"center", gap:9, padding:"10px 14px", fontSize:13, color:"#dc2626", background:"none", border:"none", cursor:"pointer", fontFamily:"inherit" }}
            ...
            <button onClick={() => setMobileOpen(v => !v)} className="mobile-menu-btn" style={{ display:"none", width:34, height:34, borderRadius:"var(--r)", border:"1.5px solid var(--border)", background:"var(--bg-alt)", cursor:"pointer", alignItems:"center", justifyContent:"center", color:"var(--text-3)" }}>
      ...
      {/* Mobile menu */}
      {mobileOpen && (
        <div style={{ position:"fixed", top:"var(--header-h)", left:0, right:0, bottom:0, background:"var(--bg-card)", zIndex:49, padding:"16px", display:"flex", flexDirection:"column", gap:6 }}>
          {NAV.map(l => (
            <Link key={l.href} href={l.href} onClick={() => setMobileOpen(false)} style={{ display:"flex", alignItems:"center", gap:10, padding:"14px 16px", borderRadius:"var(--r)", fontSize:15, fontWeight:600, color:"var(--text-2)", textDecoration:"none", background:"var(--bg-alt)" }}>
              <l.icon size={16}/>{l.label}
            </Link>
          ))}
          {!isAuth ? (
            <button onClick={() => { requireAuth(); setMobileOpen(false); }} className="btn btn-primary" style={{ marginTop:8, justifyContent:"center", height:48, fontSize:15 }}>
              <Wallet size={16}/>Sign In / Create Wallet
            </button>
          ) : (
            <Link href="/wallet-app" onClick={() => setMobileOpen(false)} className="btn btn-secondary" style={{ marginTop:8, justifyContent:"center", height:48, fontSize:15 }}>
              <Wallet size={16}/>My Wallet · ${balance}
            </Link>
          )}
        </div>
      )}
