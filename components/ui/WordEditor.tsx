...
// ── prevent editor blur before exec ───────────────────────────────
...
function Btn({ onCmd, title, active, children }: { onCmd: () => void; title: string; active?: boolean; children: React.ReactNode }) {
  return (
    <button
      ...
      style={{
        minWidth: 26, height: 26, borderRadius: 4, border: "none", cursor: "pointer",
        display: "flex", alignItems: "center", justifyContent: "center",
        background: active ? "rgba(109,40,217,.12)" : "transparent",
        color: active ? "var(--brand)" : "#374151",
...
// ── Custom dropdown (div-based, preserves editor focus) ────────────
function ToolDropdown({
  label, width, items, onSelect, onMouseDownOuter,
}: {
  label: string; width: number;
  items: { label: string; value: string; style?: React.CSSProperties }[];
  onSelect: (v: string) => void;
  onMouseDownOuter?: (e: React.MouseEvent) => void;
}) {
  ...
  return (
    <div ref={ref} style={{ position: "relative", flexShrink: 0 }}
      onMouseDown={e => { e.preventDefault(); onMouseDownOuter?.(e); }}>
      <div
        onMouseDown={e => { e.preventDefault(); setOpen(o => !o); }}
        style={{
          display: "flex", alignItems: "center", gap: 3, height: 26, padding: "0 6px 0 7px",
          background: "white", border: "1px solid #e5e7eb", borderRadius: 4,
          fontSize: 11, color: "#374151", cursor: "pointer", userSelect: "none",
      ...
      {open && (
        <div style={{
          ...
          boxShadow: "0 6px 20px rgba(0,0,0,.12)", minWidth: width + 20,
          maxHeight: 220, overflowY: "auto", padding: "3px 0",
        }}>
          {items.map(item => (
            <div
              key={item.value}
              onMouseDown={e => { e.preventDefault(); onSelect(item.value); setOpen(false); }}
              style={{
                padding: "6px 12px", fontSize: 12, cursor: "pointer",
                color: "#374151", lineHeight: 1.4, ...item.style,
              }}
...
export default function WordEditor({ value, onChange, placeholder = "Start writing…" }: Props) {
  ...
  // ── core exec — always restore focus + selection first ──────────
  ...
  // ── Font size (span-based so any pt value works) ─────────────────
  function applySize(pt: number) {
    ...
    if (!sel || sel.rangeCount === 0 || sel.isCollapsed) {
      // No selection — apply to current caret paragraph or whole editor
      el.style.fontSize = pt + "pt";
      return;
    }
    // Wrap selection in a span with exact pt size
    document.execCommand("styleWithCSS", false, "true");
    document.execCommand("fontSize", false, "7"); // marker
    el.querySelectorAll('font[size="7"]').forEach(f => {
      const span = document.createElement("span");
      span.style.fontSize = pt + "pt";
      span.style.fontFamily = f.getAttribute("face") || fontName;
  ...
  function applyHeading(tag: string, label: string) {
    setHeading(label);
    editorRef.current?.focus();
    restoreRange();
  ...
  // ── Line spacing ─────────────────────────────────────────────────
  function applySpacing(v: string) {
    setSpacing(v);
    const el = editorRef.current;
    if (!el) return;
    el.style.lineHeight = v;
    // Also update every block so it applies to existing paragraphs
    el.querySelectorAll<HTMLElement>("p,h1,h2,h3,h4,li,div,blockquote").forEach(b => {
  ...
  function insertLink() {
    restoreRange();
    if (linkUrl !== "https://") exec("createLink", linkUrl);
  ...
  function insertTbl() {
    ...
    restoreRange();
    exec("insertHTML", `<table style="border-collapse:collapse;width:100%;margin:12px 0">${row.repeat(tR)}</table><p><br></p>`);
    setShowTbl(false);
  }
    ...
    <div style={{ border: "1.5px solid var(--border)", borderRadius: "var(--r-lg)", overflow: "hidden", background: "#fff" }}>
      ...
      <div style={{ background: "#f3f4f6", borderBottom: "1px solid #e5e7eb" }}
        ...
        <div style={{ display: "flex", alignItems: "center", padding: "3px 6px", gap: 2, flexWrap: "wrap", rowGap: 2 }}>
          ...
          <Btn onCmd={() => exec("redo")} title="Redo"><Redo2 size={12} /></Btn>
          <Sep />

          {/* Heading */}
          <ToolDropdown
            label={heading} width={88}
            items={HEADINGS.map(h => ({
              label: h.l, value: h.t,
              style: h.t !== "p" ? { fontWeight: 700, fontSize: h.t === "h1" ? 16 : h.t === "h2" ? 14 : h.t === "h3" ? 13 : 12 } : {},
            }))}
            onSelect={v => applyHeading(v, HEADINGS.find(h => h.t === v)?.l || "Normal")}
          />

          {/* Font family */}
          <ToolDropdown
            label={fontName} width={120}
            items={FONTS.map(f => ({ label: f, value: f, style: { fontFamily: f } }))}
            onSelect={applyFont}
          />

          {/* Font size */}
          <ToolDropdown
            label={`${fontSize}pt`} width={58}
            items={FONT_SIZES.map(s => ({ label: `${s}pt`, value: String(s) }))}
            onSelect={v => applySize(Number(v))}
          />
          <Sep />
          ...
          <Btn onCmd={() => exec("subscript")} title="Subscript"><Subscript size={12} /></Btn>
          <Sep />

          {/* Colour */}
          <label title="Font Color" onMouseDown={pd}
            style={{ width: 26, height: 26, borderRadius: 4, cursor: "pointer", position: "relative", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
            ...
            <input type="color" defaultValue="#000000"
              ...
              style={{ opacity: 0, width: "100%", height: "100%", cursor: "pointer", border: "none", position: "absolute" }} />
          </label>
          <label title="Highlight" onMouseDown={pd}
            style={{ width: 26, height: 26, borderRadius: 4, cursor: "pointer", position: "relative", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, background: "rgba(253,230,138,.6)" }}>
          ...
          <Btn onCmd={() => exec("justifyFull")} title="Justify"><AlignJustify size={12} /></Btn>
          <Sep />
          ...
          <Btn onCmd={() => exec("formatBlock", "<blockquote>")} title="Blockquote"><Quote size={12} /></Btn>
          <Sep />
          ...
          <Btn onCmd={() => { saveRange(); setShowTbl(v => !v); setShowLink(false); setShowImg(false); }} title="Table"><Table size={12} /></Btn>
          <Sep />

          {/* Line spacing */}
          <ToolDropdown
            label={LINE_SPACINGS.find(l => l.v === spacing)?.l || spacing} width={72}
            items={LINE_SPACINGS.map(l => ({ label: l.l, value: l.v }))}
            onSelect={applySpacing}
          />

          {/* Margin */}
          <ToolDropdown
            label={MARGINS.find(m => m.v === margin)?.l || "Compact"} width={96}
            items={MARGINS.map(m => ({ label: m.l, value: m.v }))}
            onSelect={v => setMargin(v)}
          />
          <Sep />
      ...
      {showLink && (
        <div style={{ padding: "6px 10px", background: "#f9fafb", borderBottom: "1px solid #e5e7eb", display: "flex", gap: 6, flexWrap: "wrap" }}>
          <input value={linkUrl} onChange={e => setLinkUrl(e.target.value)}
            ...
            style={{ flex: 1, minWidth: 160, padding: "4px 8px", border: "1px solid #d1d5db", borderRadius: 4, fontSize: 12, outline: "none" }} />
          <button onClick={insertLink} style={{ padding: "4px 10px", background: "var(--brand)", color: "white", border: "none", borderRadius: 4, cursor: "pointer", fontSize: 11, fontWeight: 600 }}>Insert</button>
      ...
      {showImg && (
        <div style={{ padding: "6px 10px", background: "#f9fafb", borderBottom: "1px solid #e5e7eb", display: "flex", gap: 6, flexWrap: "wrap" }}>
          <input value={imgUrl} onChange={e => setImgUrl(e.target.value)}
            onKeyDown={e => e.key === "Enter" && insertImg()}
            placeholder="Image URL or upload below…" autoFocus
            style={{ flex: 1, minWidth: 120, padding: "4px 8px", border: "1px solid #d1d5db", borderRadius: 4, fontSize: 12, outline: "none" }} />
          <label style={{ display: "flex", alignItems: "center", gap: 4, padding: "4px 9px", background: "#f3f4f6", border: "1px solid #e5e7eb", borderRadius: 4, cursor: "pointer", fontSize: 11, whiteSpace: "nowrap" }}>
            ...
            <input type="file" accept="image/*" style={{ display: "none" }} onChange={e => {
              ...
              r.onload = ev => setImgUrl(ev.target?.result as string);
              r.readAsDataURL(f);
            }} />
          </label>
          <button onClick={insertImg} style={{ padding: "4px 10px", background: "var(--brand)", color: "white", border: "none", borderRadius: 4, cursor: "pointer", fontSize: 11, fontWeight: 600 }}>Insert</button>
      ...
      {showTbl && (
        <div style={{ padding: "6px 10px", background: "#f9fafb", borderBottom: "1px solid #e5e7eb", display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
          ...
          <button onClick={insertTbl} style={{ padding: "4px 10px", background: "var(--brand)", color: "white", border: "none", borderRadius: 4, cursor: "pointer", fontSize: 11, fontWeight: 600 }}>Insert Table</button>
